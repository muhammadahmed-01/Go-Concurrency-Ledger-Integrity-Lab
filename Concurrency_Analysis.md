...
## 10. The War Story (Tell It Like This)

> *"I was benchmarking a ledger endpoint — starting balance $1,000,000, 800 concurrent users each deducting $10. The buggy implementation showed zero errors, normal latency, the highest throughput of all three strategies. Perfect dashboards.*
>
> *After the run, the balance was $990,360. We'd run 160,207 deduction attempts and only 964 of them actually landed. $9,640 vanished with no alerts, no errors, no indication anything was wrong.*
>
> *The root cause was a classic read-modify-write race — two goroutines read the same balance, both passed the balance check, both wrote back a value reflecting only their deduction. The other deduction was silently overwritten.*
>
> *I tested two fixes. Pessimistic locking with `SELECT FOR UPDATE` was correct but serialized everything — 503s started flooding in at ~155/sec at peak load, p95 hit 2.2 seconds. Optimistic locking with a version column was also correct, and the version column proved it: version 12,769 matched exactly 12,768 effective deductions. But under 800 VUs on a single row, it triggered ~580,000 retries, spiked Postgres to 49.6% CPU, and p95 hit 4.25 seconds. k6 failed its own SLA thresholds.*
>
> *The key insight is that optimism scales with your conflict rate, not your VU count. It's the right tool when conflicts are rare. For a genuinely hot row — a single account everyone is hitting — you need to either serialize properly with backpressure or remove the DB from the hot path entirely with a LMAX-style sequencer.*
>
> *This maps directly to what Kleppmann covers in DDIA Chapter 7 on lost updates and compare-and-set, and the broader throughput vs consistency tension in Chapter 8."*

---

## 11. Lessons Learned

1. **Metrics can actively lie.** Buggy mode: 0% errors, green dashboards — destroying data the entire time.
2. **Completed iterations ≠ correct work.** 160,207 iterations, 964 counted. Throughput without integrity is meaningless.
3. **Optimism is fit-for-purpose.** Excellent when conflicts are rare. Catastrophic on a hot row.
4. **Retry storms outlive the load.** After k6 stopped, Go continued processing in-flight retry loops.
5. **The version column is a truth machine.** `version=12,769` with `12,768 deductions` proves correctness to within 1 operation.
6. **Context timeouts are not optional.** Without the 2s timeout, pessimistic mode causes goroutine pile-up until OOM.
7. **Always pair retries with idempotency.** OCC retries are safe here because deductions are checked. In a less careful system, retries cause double-charges.

---

*Postgres peaked at 49.6% CPU / 341.9MB RAM during optimistic mode. Go app peaked at 30.9% CPU / 63.6MB RAM.*